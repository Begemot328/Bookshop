...
INSERT INTO `genre_books` VALUES (5,6),(3,8),(4,8),(3,9),(4,9),(3,10),(4,10),(3,11),(4,11),(3,12),(4,12);
...
INSERT INTO `shops` VALUES (1,'Na Grushevke','Prospekt Dzerzhinskogo 19, Minsk 220089, Belarus',''),(2,'Na Malinovke','Praspiekt Dziaržynskaha 130, Minsk, Belarus',NULL);
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `books_genres` AS select `books`.`id` AS `id`,`books`.`title` AS `title`,`books`.`author_id` AS `author_id`,`books`.`price` AS `price`,`books`.`description` AS `description`,`books`.`photo_link` AS `photo_link`,`g`.`id` AS `genre_id` from ((`books` join `genre_books` `gb` on((`gb`.`book_id` = `books`.`id`))) join `genres` `g` on((`gb`.`genre_id` = `g`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2021-01-19 16:22:22
